...
// jit function
uint64_t (*_bfjit)(uint8_t *tape, uint64_t ptr);
uint8_t *_bfjit_mem;
...
int main(int argc, char **argv) {
    ...
    jit_append_buf(0x48, 0x89, 0xf1); // mov %rsi, %rcx
    ...
    while((c = str[i])) {
        ...
        if((c == '+' || c == '-') && (next_ch == '+' || next_ch == '-')) {
            if(do_spec_exec && 0) { // removing "&& 0" makes toh works
                while((c = str[i])) {
                    if(c == '+') tape[spec_ptr]++;
                    else if(c == '-') tape[spec_ptr]--;
                    else break;
                    i++;
                }
                continue;
        ...
        if(c == '>') {
            #ifdef OPTIMIZE
            if(do_spec_exec) spec_ptr++; // HERE?
            else
            #endif
            jit_append_buf(0x66, 0xff, 0xc1); // incw %cx
        } else if(c == '<') {
            #ifdef OPTIMIZE
            if(do_spec_exec) spec_ptr--;
            else
            #endif
            jit_append_buf(0x66, 0xff, 0xc9); // decw %cx
        } else if(c == '+') {
            #ifdef OPTIMIZE
            if(do_spec_exec) tape[spec_ptr]++;
            else
            #endif
            ...
            if(next_ch == ']') {
                 ...
                 */
                if(do_spec_exec) {
                    while(tape[spec_ptr]) {} // maybe no halt, maybe loop forever!
                }
                jit_append_buf(
                    0x8a, 0x04, 0x39,
                    0x3c, 0x00,
            ...
            do {
                ...
                while(str[j]) {
                    if(str[j] == ',') {
                        do_spec_exec = 0; // disable spec exec here
                    }
                ...
                while((c = str[i]) && i < j) {
                    if(do_spec_exec && 0) {
                        ...
                        if(c == '+' || c == '-') {
                            ...
                            //if(abs_ptr == spec_ptr) {i++; continue;}
                            tape[abs_ptr] = value_delta*tape[spec_ptr];
                            //printf("L: %d\n",tape[spec_ptr]);
                        }
                        else if (c=='>') abs_ptr++;
                        else if (c=='<') abs_ptr--;
                        ...
                        if(c == '+' || c == '-') {
                            ...
                            // *(tape+ptr+rela_ptr) = value_delta*tape[ptr]
            ...
            do_spec_exec = 0; // not a simple loop
